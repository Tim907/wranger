...
TreeClassification::TreeClassification(std::vector<double>* class_values, std::vector<uint64_t>* response_classIDs,
...
TreeClassification::TreeClassification(std::vector<std::vector<size_t>>& child_nodeIDs,
    ...
    std::vector<uint64_t>* response_classIDs) :
...
double TreeClassification::estimate(size_t nodeID) {
  ...
  for (size_t pos = start_pos[nodeID]; pos < end_pos[nodeID]; ++pos) {
    ...
    class_count[value] += (*class_weights)[value] * data->get_w(sampleID, 0);
  }
  // dividing by sum weights unnecessary
...
double TreeClassification::computePredictionAccuracyInternal(std::vector<double>* prediction_error_casewise) {
  ...
  for (size_t i = 0; i < num_predictions; ++i) {
    ...
    double real_value = data->get_y(oob_sampleIDs[i], 0) * data->get_w(oob_sampleIDs[i], 0);
