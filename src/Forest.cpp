...
// #nocov start
void Forest::initCpp(std::string dependent_variable_name, MemoryMode memory_mode, std::string input_file, uint64_t mtry,
    std::string output_prefix, uint64_t num_trees, std::ostream* verbose_out, uint64_t seed, uint64_t num_threads,
    std::string load_forest_filename, ImportanceMode importance_mode, uint64_t min_node_size, uint64_t min_bucket,
    ...
    std::string obs_weights_file, std::string case_weights_file, bool predict_all, double sample_fraction, double alpha, double minprop, bool holdout,
    PredictionType prediction_type, uint64_t num_random_splits, uint64_t max_depth,
  ...
  // Load observation weights from file
  if (!case_weights_file.empty()) {
    loadDoubleVectorFromFile(obs_weights, obs_weights_file);
    if (obs_weights.size() != num_samples) {
      throw std::runtime_error("Number of observation weights is not equal to number of samples.");
    }
  }
...
void Forest::initR(std::unique_ptr<Data> input_data, uint64_t mtry, uint64_t num_trees, std::ostream* verbose_out, uint64_t seed,
    uint64_t num_threads, ImportanceMode importance_mode, uint64_t min_node_size, uint64_t min_bucket,
    ...
    uint64_t num_random_splits, bool order_snps, uint64_t max_depth, const std::vector<double>& regularization_factor,
...
void Forest::init(std::unique_ptr<Data> input_data, uint64_t mtry, std::string output_prefix,
    uint64_t num_trees, uint64_t seed, uint64_t num_threads, ImportanceMode importance_mode, uint64_t min_node_size, uint64_t min_bucket,
    bool prediction_mode, bool sample_with_replacement, const std::vector<std::string>& unordered_variable_names,
    bool memory_saving_splitting, SplitRule splitrule, bool predict_all, std::vector<double>& sample_fraction,
    double alpha, double minprop, bool holdout, PredictionType prediction_type, uint64_t num_random_splits, bool order_snps,
    uint64_t max_depth, const std::vector<double>& regularization_factor, bool regularization_usedepth) {
...
void Forest::saveToFile() {
  ...
  uint64_t num_dependent_variables = dependent_variable_names.size();
...
void Forest::growTreesInThread(uint64_t thread_idx, std::vector<double>* variable_importance) {
...
void Forest::predictTreesInThread(uint64_t thread_idx, const Data* prediction_data, bool oob_prediction) {
  if (thread_ranges.size() > thread_idx + 1) {
    for (size_t i = thread_ranges[thread_idx]; i < thread_ranges[thread_idx + 1]; ++i) {
...
void Forest::predictInternalInThread(uint64_t thread_idx) {
  // Create thread ranges
  std::vector<uint64_t> predict_ranges;
...
void Forest::computeTreePermutationImportanceInThread(uint64_t thread_idx, std::vector<double>& importance,
...
void Forest::loadFromFile(std::string filename) {
  ...
  uint64_t num_dependent_variables;
...
void Forest::loadDependentVariableNamesFromFile(std::string filename) {
  ...
  uint64_t num_dependent_variables = 0;
  infile.read((char*) &num_dependent_variables, sizeof(num_dependent_variables));
  for (size_t i = 0; i < num_dependent_variables; ++i) {
    size_t length;
    infile.read((char*) &length, sizeof(size_t));
...
void Forest::showProgress(std::string operation, size_t max_progress) {
  ...
  while (progress < max_progress) {
    ...
    if (progress > 0 && elapsed_time.count() > STATUS_INTERVAL) {
      ...
      uint64_t remaining_time = (1 / relative_progress - 1) * time_from_start.count();
      if (verbose_out) {
