...
double TreeRegression::computePredictionAccuracyInternal(std::vector<double>* prediction_error_casewise) {
  ...
  for (size_t i = 0; i < num_predictions; ++i) {
    ...
    if (predicted_value != real_value) {
      double diff = (predicted_value - real_value) * (predicted_value - real_value) * data->get_w(oob_sampleIDs[i], 0) / sum_weights;
  ...
  // commented because already divided by sum_weights above
...
bool TreeRegression::findBestSplit(size_t nodeID, std::vector<size_t>& possible_split_varIDs) {
  ...
  if (num_samples_node >= 2 * min_bucket) {
    ...
    for (auto& varID : possible_split_varIDs) {
      ...
      findBestSplitValueLargeQ(nodeID, varID, sum_node, cost_node, num_samples_node, sum_node_weights, best_value, best_varID, best_decrease);
    }
  }
...
void TreeRegression::findBestSplitValueSmallQ(size_t nodeID, size_t varID, double sum_node, size_t num_samples_node,
  ...
  for (size_t pos = start_pos[nodeID]; pos < end_pos[nodeID]; ++pos) {
    ...
    sums[idx] += data->get_y(sampleID, 0) * data->get_w(sampleID, 0);
    ++counter[idx];
  }
...
void TreeRegression::findBestSplitValueLargeQ(size_t nodeID, size_t varID, double sum_node, double cost_node, size_t num_samples_node, double sum_node_weights,
    double& best_value, size_t& best_varID, double& best_decrease) {
  ...
  for (size_t pos = start_pos[nodeID]; pos < end_pos[nodeID]; ++pos) {
    ...
    // printf("%lf | %lf | %lf \n", data->get_y(sampleID, 0), data->get_w(sampleID, 0), data->get_y(sampleID, 0) * data->get_w(sampleID, 0));
  ...
  for (size_t i = 0; i < num_unique - 1; ++i) {
    ...
    // double decrease = sum_left * sum_left / (double) n_left + sum_right * sum_right / (double) n_right;
    // double decrease = cost_left / weight_left + cost_right / weight_right;
...
void TreeRegression::findBestSplitValueUnordered(size_t nodeID, size_t varID, double sum_node, size_t num_samples_node,
  ...
  for (size_t local_splitID = 1; local_splitID < num_splits / 2; ++local_splitID) {
    ...
    for (size_t pos = start_pos[nodeID]; pos < end_pos[nodeID]; ++pos) {
      size_t sampleID = sampleIDs[pos];
      double response = data->get_y(sampleID, 0) * data->get_w(sampleID, 0);
...
bool TreeRegression::findBestSplitMaxstat(size_t nodeID, std::vector<size_t>& possible_split_varIDs) {
  ...
  for (size_t pos = start_pos[nodeID]; pos < end_pos[nodeID]; ++pos) {
    size_t sampleID = sampleIDs[pos];
    response.push_back(data->get_y(sampleID, 0) * data->get_w(sampleID, 0));
  }
...
void TreeRegression::findBestSplitValueBeta(size_t nodeID, size_t varID, double sum_node, size_t num_samples_node,
  ...
  for (size_t i = 0; i < num_splits; ++i) {
    ...
    throw std::runtime_error("unimplemented with weighting");
    // Compute variance
    double var_right = 0;
    double var_left = 0;
    for (size_t pos = start_pos[nodeID]; pos < end_pos[nodeID]; ++pos) {
      ...
      if (value > possible_split_values[i]) {
        var_right += (response - mean_right) * (response - mean_right) * data->get_w(sampleID, 0);
      } else {
        var_left += (response - mean_left) * (response - mean_left) * data->get_w(sampleID, 0);
      }
    }
