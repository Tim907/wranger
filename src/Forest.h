...
public:
  ...
  void initCpp(std::string dependent_variable_name, MemoryMode memory_mode, std::string input_file, uint64_t mtry,
      ...
      std::string obs_weights_file, std::string case_weights_file, bool predict_all, double sample_fraction, double alpha, double minprop,
      bool holdout, PredictionType prediction_type, uint64_t num_random_splits, uint64_t max_depth,
      const std::vector<double>& regularization_factor, bool regularization_usedepth);
  void initR(std::unique_ptr<Data> input_data, uint64_t mtry, uint64_t num_trees, std::ostream* verbose_out, uint64_t seed,
      ...
      PredictionType prediction_type, uint64_t num_random_splits, bool order_snps, uint64_t max_depth,
      const std::vector<double>& regularization_factor, bool regularization_usedepth);
  void init(std::unique_ptr<Data> input_data, uint64_t mtry, std::string output_prefix,
      ...
      double alpha, double minprop, bool holdout, PredictionType prediction_type, uint64_t num_random_splits,
      bool order_snps, uint64_t max_depth, const std::vector<double>& regularization_factor, bool regularization_usedepth);
  ...
  uint64_t getMtry() const {
    return mtry;
  }
  uint64_t getMinNodeSize() const {
    return min_node_size;
  }
  uint64_t getMinBucket() const {
    return min_bucket;
  }
...
protected:
  ...
  void growTreesInThread(uint64_t thread_idx, std::vector<double>* variable_importance);
  void predictTreesInThread(uint64_t thread_idx, const Data* prediction_data, bool oob_prediction);
  void predictInternalInThread(uint64_t thread_idx);
  void computeTreePermutationImportanceInThread(uint64_t thread_idx, std::vector<double>& importance,
  ...
  std::vector<uint64_t> thread_ranges;
  std::mutex mutex;
