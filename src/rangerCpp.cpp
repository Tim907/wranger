...
// [[Rcpp::export]]
Rcpp::List rangerCpp(uint64_t treetype, Rcpp::NumericMatrix& input_x, Rcpp::NumericMatrix& input_y, Rcpp::NumericMatrix& obs_weights,
    std::vector<std::string> variable_names, uint64_t mtry, uint64_t num_trees, bool verbose, uint64_t seed, uint64_t num_threads,
    bool write_forest, uint64_t importance_mode_r, uint64_t min_node_size, uint64_t min_bucket,
    ...
    bool use_unordered_variable_names, bool save_memory, uint64_t splitrule_r,
    std::vector<double>& case_weights,
    bool use_case_weights, std::vector<double>& class_weights, bool predict_all, bool keep_inbag,
    std::vector<double>& sample_fraction, double alpha, double minprop, bool holdout, uint64_t prediction_type_r,
    uint64_t num_random_splits, Eigen::SparseMatrix<double>& sparse_x,
    bool use_sparse_data, bool order_snps, bool oob_error, uint64_t max_depth,
  ...
  try {
    ...
    // Initialize data
    if (use_sparse_data) {
      data = std::make_unique<DataSparse>(sparse_x, input_y, obs_weights, variable_names, num_rows, num_cols);
    } else {
      data = std::make_unique<DataRcpp>(input_x, input_y, obs_weights, variable_names, num_rows, num_cols);
    }
    ...
    forest->initR(std::move(data), mtry, num_trees, verbose_out, seed, num_threads,
        ...
        case_weights, inbag, predict_all, keep_inbag, sample_fraction, alpha, minprop, holdout,
        prediction_type, num_random_splits, order_snps, max_depth, regularization_factor, regularization_usedepth);
