...
void ForestRegression::computePredictionErrorInternal() {
  ...
  for (size_t i = 0; i < predictions[0][0].size(); ++i) {
    if (samples_oob_count[i] > 0) {
      ...
      overall_prediction_error += data->get_w(i, 0) / sum_weights * (predicted_value - real_value) * (predicted_value - real_value);
    } else {
      predictions[0][0][i] = NAN;
    }
  }

  // comment because division by sum_weights
