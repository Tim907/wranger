...
void equalSplit(std::vector<uint64_t>& result, uint64_t start, uint64_t end, uint64_t num_parts) {
  ...
  uint64_t part_length_short = length / num_parts;
  uint64_t part_length_long = (uint64_t) ceil(length / ((double) num_parts));
  uint64_t cut_pos = length % num_parts;

  // Add long ranges
  for (uint64_t i = start; i < start + cut_pos * part_length_long; i = i + part_length_long) {
    result.push_back(i);
  }

  // Add short ranges
  for (uint64_t i = start + cut_pos * part_length_long; i <= end + 1; i = i + part_length_short) {
    result.push_back(i);
  }
}
...
std::string uint64_tToString(uint64_t number) {
#if WIN_R_BUILD == 1
...
std::string beautifyTime(uint64_t seconds) { // #nocov start
  std::string result;

  // Add seconds, minutes, hours, days if larger than zero
  uint64_t out_seconds = (uint64_t) seconds % 60;
  result = uint64_tToString(out_seconds) + " seconds";
  uint64_t out_minutes = (seconds / 60) % 60;
  if (seconds / 60 == 0) {
    return result;
  } else if (out_minutes == 1) {
    result = "1 minute, " + result;
  } else {
    result = uint64_tToString(out_minutes) + " minutes, " + result;
  }
  uint64_t out_hours = (seconds / 3600) % 24;
  if (seconds / 3600 == 0) {
    return result;
  } else if (out_hours == 1) {
    result = "1 hour, " + result;
  } else {
    result = uint64_tToString(out_hours) + " hours, " + result;
  }
  uint64_t out_days = (seconds / 86400);
  if (out_days == 0) {
    return result;
  } else if (out_days == 1) {
    result = "1 day, " + result;
  } else {
    result = uint64_tToString(out_days) + " days, " + result;
  }
  return result;
} // #nocov end

// #nocov start
size_t roundToNextMultiple(size_t value, uint64_t multiple) {
  ...
  for (auto& variable_name : unordered_variable_names) {
    ...
    if (all_values.size() > max_level_count) {
      return "Too many levels in unordered categorical variable " + variable_name + ". Only "
          + uint64_tToString(max_level_count) + " levels allowed on this system.";
    }
