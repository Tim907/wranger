...
TreeProbability::TreeProbability(std::vector<double>* class_values, std::vector<uint64_t>* response_classIDs,
...
TreeProbability::TreeProbability(std::vector<std::vector<size_t>>& child_nodeIDs, std::vector<size_t>& split_varIDs,
    std::vector<double>& split_values, std::vector<double>* class_values, std::vector<uint64_t>* response_classIDs,
...
bool TreeProbability::findBestSplit(size_t nodeID, std::vector<size_t>& possible_split_varIDs) {
  ...
  for (size_t pos = start_pos[nodeID]; pos < end_pos[nodeID]; ++pos) {
    size_t sampleID = sampleIDs[pos];
    uint64_t sample_classID = (*response_classIDs)[sampleID];
  ...
  if (num_samples_node >= 2 * min_bucket) {
    ...
    for (auto& varID : possible_split_varIDs) {
      ...
      findBestSplitValueLargeQ(nodeID, varID, num_classes, class_counts, cost_node, num_samples_node, sum_node_weights, best_value, best_varID, best_decrease);
    }
  }
...
void TreeProbability::findBestSplitValueLargeQ(size_t nodeID, size_t varID, size_t num_classes,
  ...
  for (size_t i = 0; i < num_unique - 1; ++i) {
    ...
    // Check if \hat\nu_k sum to 1
    // printf("%lf \n", nu_k_left[0] + nu_k_left[1]);
    // printf("%lf \n", nu_k_right[0] + nu_k_right[1]);
    // printf("%lf %lf \n", sum_right, sum_left);

    // Decrease of impurity
    // compare old cost with new cost
