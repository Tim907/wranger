...
##' @param obs.weights Weights for the contribution to the loss function of training observations. Observations with larger weights will have more impact in the node splitting rule for the trees.
...
##' @param splitrule Splitting rule. For classification and probability estimation "gini". For regression "variance". For survival "logrank", "extratrees", "C" or "maxstat" with default "logrank".
...
ranger <- function(formula = NULL, data = NULL, num.trees = 500, mtry = NULL,
                   ...
                   obs.weights = NULL, case.weights = NULL, class.weights = NULL,
  ...
  ###### Parameters for the implementations of obs.weights
  # Handle weight related warnings
  if (!is.null(obs.weights) & !is.null(case.weights)) {
    stop("Error: Either obs.weights or case.weights is required.")
  }
  ...
  ##### Treetype #####
  # survival treetype = 5
  # regression treetype = 3
  # classification treetype %in% c(1, 9)
  # 1 for classification forest, 9 for probability forest
  # Only allow probability forest
  ...
  if (is.null(splitrule)) {
    if (treetype == 5) { # survival
      splitrule <- "logrank"
    } else if (treetype == 3) { # regression
      splitrule <- "variance"
    } else if (treetype %in% c(1, 9)) { # classification of probability forest
      splitrule <- "gini"
    }
    splitrule.num <- 1
    ...
    if (treetype == 5) {
      ...
      stop("Error: maxstat splitrule applicable to survival data only.")
    }
  ...
  obs.weights.mat = as.matrix(obs.weights)
